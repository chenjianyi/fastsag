...
class PerceiverIO(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.queries = nn.Parameter(torch.randn(queries_num, queries_dim))
...
if __name__ == '__main__':
    ...
    mask = torch.tensor([[1] * 256 + [0] * 256]).bool()
    print(mask.size())
    
    logits = model(seq, queries = queries, mask=mask) # (1, 128, 100) - (batch, decoder seq, logits dim)
    print(seq.size(), logits.size())
