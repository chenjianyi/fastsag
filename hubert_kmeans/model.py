...
from .utils import exists, curtail_to_multiple, zero_mean_unit_var_norm, beartype_jit, disable_print
...
hubert_kmeans_cfg = {
    "model_name": "m-a-p/MERT-v1-95M", #"m-a-p/MERT-v0",
    "normalize_embeds": True,
    "embed_layer": 7,
    "target_sample_hz": 24000, #16000,
...
@beartype_jit
def create_hubert_kmeans_from_config(kmeans_path: Optional[str], device, **kwargs):
...
def get_hubert_kmeans(model_name: str="m-a-p/MERT-v0", kmeans_path='./checkpoints/kmeans.joblib', **kwargs):
    wav2vec = HubertModel.from_pretrained(model_name)
    kmeans = joblib.load(kmeans_path) if kmeans_path is not None and os.path.exists(kmeans_path) else None
